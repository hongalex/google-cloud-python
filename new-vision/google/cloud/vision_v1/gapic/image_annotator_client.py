...
class ImageAnnotatorClient(object):
    ...
    def batch_annotate_files(self,
        ...
        if 'batch_annotate_files' not in self._inner_api_calls:
            self._inner_api_calls[
                'batch_annotate_files'] = google.api_core.gapic_v1.method.wrap_method(
                    self.transport.batch_annotate_files,
                    default_retry=self._method_configs['BatchAnnotateFiles'].
                    retry,
                    default_timeout=self._method_configs['BatchAnnotateFiles'].
                    timeout,
                    client_info=self._client_info,
                )
        ...
        request = image_annotator_pb2.BatchAnnotateFilesRequest(
            requests=requests, )
        return self._inner_api_calls['batch_annotate_files'](request,
                                                             retry=retry,
                                                             timeout=timeout,
                                                             metadata=metadata)
    ...
    def async_batch_annotate_images(
        ...
        if 'async_batch_annotate_images' not in self._inner_api_calls:
            self._inner_api_calls[
                'async_batch_annotate_images'] = google.api_core.gapic_v1.method.wrap_method(
                    self.transport.async_batch_annotate_images,
                    default_retry=self.
                    _method_configs['AsyncBatchAnnotateImages'].retry,
                    default_timeout=self.
                    _method_configs['AsyncBatchAnnotateImages'].timeout,
                    client_info=self._client_info,
                )
